...
function read_source()
	local config = {} -- Contain the list of the file used for analyze
	local p = io.popen('find "/etc/mkapt/" --type=f')  --Open directory look for files     
	for file in p:lines() do                         -- Recovering all the file name in the folder 
		print(file)       
		table.insert(config, file)
	end

	for cle, valeur in ipairs(config) do
		lines = {}
		for line in io.lines(valeur) do 
			lines[#lines + 1] = line
		end
	end
